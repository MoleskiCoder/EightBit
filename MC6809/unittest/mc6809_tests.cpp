#include "pch.h"
#include "Board.h"

// Some tests from:
// 6809 Assembly Language Programming, by Lance A. Leventhal
// https://github.com/sorenroug/osnine-java/blob/master/core/cputests/

BOOST_AUTO_TEST_SUITE(ABX)

BOOST_AUTO_TEST_CASE(Inherent) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x3a);
	cpu.B() = 0x84;
	cpu.X() = 0x1097;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.X(), 0x111b);
	BOOST_CHECK_EQUAL(cycles, 3);
}

BOOST_AUTO_TEST_CASE(ABX1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0;
	cpu.B() = 0xce;
	cpu.X() = 0x8006;
	cpu.Y() = 0;
	cpu.U() = 0;
	cpu.CC() = 0;
	board.poke(0, 0x3a);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0);
	BOOST_CHECK_EQUAL(cpu.B(), 0xce);
	BOOST_CHECK_EQUAL(cpu.X(), 0x80d4);
	BOOST_CHECK_EQUAL(cpu.Y(), 0);
	BOOST_CHECK_EQUAL(cpu.U(), 0);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 3);
}

BOOST_AUTO_TEST_CASE(ABX2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0;
	cpu.B() = 0xd6;
	cpu.X() = 0x7ffe;
	cpu.Y() = 0;
	cpu.U() = 0;
	cpu.CC() = EightBit::mc6809::CF | EightBit::mc6809::VF | EightBit::mc6809::ZF;
	board.poke(0, 0x3a);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0);
	BOOST_CHECK_EQUAL(cpu.B(), 0xd6);
	BOOST_CHECK_EQUAL(cpu.X(), 0x80d4);
	BOOST_CHECK_EQUAL(cpu.Y(), 0);
	BOOST_CHECK_EQUAL(cpu.U(), 0);
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 3);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(ADC)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x89);
	board.poke(1, 0x7c);
	cpu.CC() = EightBit::Chip::setBit(cpu.CC(), EightBit::mc6809::CF);
	cpu.A() = 0x3a;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xb7);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.halfCarry());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_CASE(ADCANoC1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0x5;
	cpu.CC() = 0;
	board.poke(0, 0x89);
	board.poke(1, 0x02);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 7);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Test half-carry $E + $2 = $10
BOOST_AUTO_TEST_CASE(ADCANoC2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0xe;
	cpu.CC() = 0;
	board.poke(0, 0x89);
	board.poke(1, 0x02);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x10);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Add $22 and carry to register A ($14)
BOOST_AUTO_TEST_CASE(ADCAWiC) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0x14;
	cpu.CC() = EightBit::mc6809::CF | EightBit::mc6809::HF;
	board.poke(0, 0x89);
	board.poke(1, 0x22);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x37);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Test that half-carry is set when adding with a carry
BOOST_AUTO_TEST_CASE(ADCAWiHC) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	cpu.A() = 0x14;
	cpu.CC() = EightBit::mc6809::CF;
	board.poke(0, 0x89);
	board.poke(1, 0x2B);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x40);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(cpu.halfCarry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(ADD)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x8b);
	board.poke(1, 0x8b);
	cpu.A() = 0x24;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xaf);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Add 0x02 to A=0x04.
BOOST_AUTO_TEST_CASE(ADDANoC) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x8b);
	board.poke(1, 0x02);
	cpu.CC() = 0;
	cpu.A() = 4;
	cpu.B() = 5;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 6);
	BOOST_CHECK_EQUAL(cpu.B(), 5);
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// A=0x03 + 0xFF becomes 0x02
BOOST_AUTO_TEST_CASE(ADDAWiC) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x8b);
	board.poke(1, 0xff);
	cpu.CC() = 0;
	cpu.A() = 3;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// positive + positive with overflow: B=0x40 + 0x41 becomes 0x81
BOOST_AUTO_TEST_CASE(ADDB1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xcb);
	board.poke(1, 0x41);
	cpu.B() = 0x40;
	cpu.CC() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0x81);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.halfCarry());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// negative + negative: B=0xFF + 0xFF becomes 0xFE
BOOST_AUTO_TEST_CASE(ADDB2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xcb);
	board.poke(1, 0xff);
	cpu.B() = 0xff;
	cpu.CC() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0xfe);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// negative + negative with overflow: B=0xC0 + 0xBF becomes 0x7F
BOOST_AUTO_TEST_CASE(ADDB3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xcb);
	board.poke(1, 0xbf);
	cpu.B() = 0xc0;
	cpu.CC() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0x7f);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// positive + negative with negative result: B=0x02 + 0xFC becomes 0xFE
BOOST_AUTO_TEST_CASE(ADDB4) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xcb);
	board.poke(1, 0xfc);
	cpu.B() = 0x02;
	cpu.CC() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0xfe);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Add 0x02B0 to D=0x0405 becomes 0x6B5
BOOST_AUTO_TEST_CASE(ADDDNoC) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc3);
	board.poke(1, 0x02);
	board.poke(2, 0xb0);
	cpu.CC() = 0;
	cpu.A() = 4;
	cpu.B() = 5;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x06);
	BOOST_CHECK_EQUAL(cpu.B(), 0xb5);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 4);
}

// Add 0xE2B0 to D=0x8405 becomes 0x66B5
BOOST_AUTO_TEST_CASE(ADDD1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc3);
	board.poke(1, 0xe2);
	board.poke(2, 0xb0);
	cpu.CC() = 0;
	cpu.A() = 0x84;
	cpu.B() = 5;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x66);
	BOOST_CHECK_EQUAL(cpu.B(), 0xb5);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 4);
}

// Add 0xE000 to D=0xD000 becomes 0xB000
BOOST_AUTO_TEST_CASE(ADDD2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc3);
	board.poke(1, 0xe0);
	board.poke(2, 0);
	cpu.CC() = 0;
	cpu.A() = 0xd0;
	cpu.B() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xb0);
	BOOST_CHECK_EQUAL(cpu.B(), 0x00);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cpu.cycles(), 4);
}

// Add 0x7000 to D=0x7000 becomes 0xE000
BOOST_AUTO_TEST_CASE(ADDD3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc3);
	board.poke(1, 0x70);
	board.poke(2, 0);
	cpu.CC() = 0;
	cpu.A() = 0x70;
	cpu.B() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xe0);
	BOOST_CHECK_EQUAL(cpu.B(), 0x00);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cpu.cycles(), 4);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(AND_Accumulator)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x84);
	board.poke(1, 0x13);
	cpu.A() = 0xfc;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x10);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(ASL)

BOOST_AUTO_TEST_CASE(Inherent) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x48);
	cpu.A() = 0x7a;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xf4);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(ASR)

BOOST_AUTO_TEST_CASE(Inherent) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x47);
	cpu.A() = 0xcb;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xe5);
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(BIT_Test)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x85);
	board.poke(1, 0xe0);
	cpu.A() = 0xa6;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xa6);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(CLR)

BOOST_AUTO_TEST_CASE(Implied) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4f);
	cpu.A() = 0x43;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x00);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(CMP)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x81);
	board.poke(1, 0x18);
	cpu.A() = 0xf6;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xf6);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_CASE(CMP1_indexed) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xa1);
	board.poke(1, 0xa0);
	board.poke(0x205, 0xff);
	cpu.CC() = 0;
	cpu.A() = 0xff;
	cpu.B() = 0;
	cpu.X() = 0;
	cpu.Y() = 0x205;
	cpu.U() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xff);
	BOOST_CHECK_EQUAL(cpu.B(), 0x00);
	BOOST_CHECK_EQUAL(cpu.X(), 0x00);
	BOOST_CHECK_EQUAL(cpu.Y(), 0x206);
	BOOST_CHECK_EQUAL(cpu.U(), 0);
	BOOST_CHECK(cpu.zero() != 0);
	BOOST_CHECK_EQUAL(cycles, 6);
}

// B = 0xA0, CMPB with 0xA0
BOOST_AUTO_TEST_CASE(CMP2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc1);
	board.poke(1, 0xa0);
	cpu.CC() = EightBit::mc6809::NF;
	cpu.B() = 0xa0;
	cycles = cpu.step();
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// B = 0x70, CMPB with 0xA0
BOOST_AUTO_TEST_CASE(CMP3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc1);
	board.poke(1, 0xa0);
	cpu.CC() = EightBit::mc6809::NF;
	cpu.B() = 0x70;
	cycles = cpu.step();
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Compare 0x5410 with 0x5410
BOOST_AUTO_TEST_CASE(CMP16) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc1);
	board.poke(1, 0xa0);
	cpu.CC() = EightBit::mc6809::NF;
	cpu.B() = 0x70;
	cycles = cpu.step();
	BOOST_CHECK(cpu.carry() != 0);
	BOOST_CHECK(cpu.overflow() != 0);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_CASE(Immediate_word) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x8c);
	board.poke(1, 0x1b);
	board.poke(2, 0xb0);
	cpu.X() = 0x1ab0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.X(), 0x1ab0);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 4);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(DEC)

BOOST_AUTO_TEST_CASE(DECA0x32) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4a);
	cpu.CC() = 0;
	cpu.A() = 0x32;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x31);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Test 0x80 - special case
BOOST_AUTO_TEST_CASE(DECA0x80) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4a);
	cpu.CC() = 0;
	cpu.A() = 0x80;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x7f);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// Test 0x00 - special case
BOOST_AUTO_TEST_CASE(DECA0x00) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4a);
	cpu.CC() = 0;
	cpu.A() = 0x00;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xff);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(INC)

BOOST_AUTO_TEST_CASE(INCA1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4c);
	cpu.CC() = 0;
	cpu.A() = 0x32;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x33);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_CASE(INCA2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4c);
	cpu.CC() = 0;
	cpu.A() = 0x7f;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0x80);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_CASE(INCA3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x4c);
	cpu.CC() = 0;
	cpu.A() = 0xff;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(SBC)

BOOST_AUTO_TEST_CASE(Immediate_byte) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x82);
	board.poke(1, 0x34);
	cpu.A() = 0x14;
	cpu.CC() = cpu.setBit(cpu.CC(), EightBit::mc6809::CF);
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xdf);
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// B=0x35 - addr(0x503)=0x3 - C=1 becomes 0x31
BOOST_AUTO_TEST_CASE(SBCB_direct) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xd2);
	board.poke(1, 0x03);
	board.poke(0x503, 0x03);
	cpu.DP() = 5;
	cpu.B() = 0x35;
	cpu.CC() = EightBit::mc6809::CF;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0x31);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 4);
}

// A=0xFF - 0xFE - C=1 becomes 0x00
BOOST_AUTO_TEST_CASE(SBCA1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x82);
	board.poke(1, 0xfe);
	board.poke(0x503, 0x03);
	cpu.CC() = EightBit::mc6809::CF | EightBit::mc6809::NF;
	cpu.A() = 0xff;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0);
	BOOST_CHECK(!cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// A=0x00 - 0xFF - C=0 becomes 0x01
BOOST_AUTO_TEST_CASE(SBCA2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x82);
	board.poke(1, 0xff);
	cpu.CC() = EightBit::mc6809::NF | EightBit::mc6809::VF;
	cpu.A() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// A=0x00 - 0x01 - C=0 becomes 0xFF
BOOST_AUTO_TEST_CASE(SBCA3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x82);
	board.poke(1, 0x01);
	cpu.CC() = EightBit::mc6809::NF | EightBit::mc6809::VF;
	cpu.A() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xff);
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(SUB)

// A=0x00 - 0xFF becomes 0x01
BOOST_AUTO_TEST_CASE(SUBA1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x80);
	board.poke(1, 0xff);
	cpu.CC() = EightBit::mc6809::CF | EightBit::mc6809::NF | EightBit::mc6809::VF;
	cpu.A() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// A=0x00 - 0x01 becomes 0xFF
BOOST_AUTO_TEST_CASE(SUBA2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x80);
	board.poke(1, 1);
	cpu.CC() = EightBit::mc6809::CF | EightBit::mc6809::NF | EightBit::mc6809::VF;
	cpu.A() = 0;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 0xff);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// B=0x02 - 0xB3 becomes 0x4F
BOOST_AUTO_TEST_CASE(SUBB1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc0);
	board.poke(1, 0xb3);
	cpu.CC() = 0;
	cpu.B() = 2;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0x4f);
	BOOST_CHECK(!cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// B=0x02 - 0x81 becomes 0x81
BOOST_AUTO_TEST_CASE(SUBB2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xc0);
	board.poke(1, 0x81);
	cpu.CC() = 0;
	cpu.B() = 2;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0x81);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 2);
}

// 0x03 - 0x21 = 0xE2 (indexed: SUBB ,Y)
BOOST_AUTO_TEST_CASE(SUBBY) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0xe0);
	board.poke(1, 0xa4);
	board.poke(0x21, 0x21);
	cpu.CC() = EightBit::mc6809::ZF;
	cpu.B() = 3;
	cpu.Y() = 0x21;
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.B(), 0xe2);
	BOOST_CHECK(cpu.negative());
	BOOST_CHECK(!cpu.zero());
	BOOST_CHECK(!cpu.overflow());
	BOOST_CHECK(cpu.carry());
	BOOST_CHECK_EQUAL(cycles, 4);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(BGT)

BOOST_AUTO_TEST_CASE(BGT1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2e);	// BGT
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::ZF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_CASE(BGT2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2e);	// BGT
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.PC(), 0);
	cpu.CC() = 0;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
}

BOOST_AUTO_TEST_CASE(BGT3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2e);	// BGT
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	BOOST_REQUIRE_EQUAL(cpu.PC(), 0);
	cpu.CC() = EightBit::mc6809::NF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_CASE(BGT4) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2e);	// BGT
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.PC(), 0);
	cpu.CC() = EightBit::mc6809::NF | EightBit::mc6809::VF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
}

BOOST_AUTO_TEST_CASE(BGT5) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2e);	// BGT
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.PC(), 0);
	cpu.CC() = EightBit::mc6809::ZF | EightBit::mc6809::NF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(BHI)

BOOST_AUTO_TEST_CASE(BHI1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x22);	// BHI
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::ZF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(BLE)

BOOST_AUTO_TEST_CASE(BLE1) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2f);	// BLE
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::ZF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
}

BOOST_AUTO_TEST_CASE(BLE2) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2f);	// BLE
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.A() = 0;
	cpu.CC() = 0;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_CASE(BLE3) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2f);	// BLE
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::NF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
}

BOOST_AUTO_TEST_CASE(BLE4) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2f);	// BLE
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::NF | EightBit::mc6809::VF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 1);
}

BOOST_AUTO_TEST_CASE(BLE5) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset
	board.poke(0, 0x2f);	// BLE
	board.poke(1, 0x03);
	board.poke(2, 0x86);	// LDA #1
	board.poke(3, 0x01);
	board.poke(4, 0x12);	// NOP
	board.poke(5, 0x86);	// LDA #2
	board.poke(6, 0x02);
	board.poke(7, 0x12);	// NOP
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.lowerRESET();
	cycles = cpu.step();
	cpu.A() = 0;
	cpu.CC() = EightBit::mc6809::ZF | EightBit::mc6809::NF;
	cycles = cpu.step();
	cycles = cpu.step();
	BOOST_CHECK_EQUAL(cpu.A(), 2);
}

BOOST_AUTO_TEST_SUITE_END()

//

BOOST_AUTO_TEST_SUITE(Subroutines)

// JSR D,Y (indexed)
BOOST_AUTO_TEST_CASE(JSR) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset

	cpu.pokeShort(0x205, 0x03ff);
	cpu.D() = 0x105;
	cpu.Y() = 0x200;
	cpu.S() = 0x915;
	board.poke(0xB00, 0xAD);
	board.poke(0xB01, 0xAB);
	board.poke(0xB02, 0x11);	// Junk
	board.poke(0xB03, 0x22);	// Junk
	cpu.PC() = 0xB00;
	cpu.CC() = 0;
	cycles = cpu.step();

	BOOST_CHECK_EQUAL(cpu.CC(), 0);
	BOOST_CHECK_EQUAL(cpu.A(), 1);
	BOOST_CHECK_EQUAL(cpu.B(), 5);
	BOOST_CHECK_EQUAL(cpu.DP(), 0);
	BOOST_CHECK_EQUAL(cpu.X(), 0);
	BOOST_CHECK_EQUAL(cpu.Y(), 0x200);
	BOOST_CHECK_EQUAL(cpu.D(), 0x105);
	BOOST_CHECK_EQUAL(cpu.S(), 0x913);
	BOOST_CHECK_EQUAL(cpu.U(), 0);
	BOOST_CHECK_EQUAL(cpu.PC(), 0x305);
	BOOST_CHECK_EQUAL(board.peek(0x914), 2);
	BOOST_CHECK_EQUAL(board.peek(0x913), 0xb);
	BOOST_CHECK_EQUAL(cycles, 11);
}

BOOST_AUTO_TEST_CASE(RTS) {
	Board board;
	board.raisePOWER();
	auto& cpu = board.CPU();
	auto cycles = cpu.step();	// Step over the reset

	cpu.lowerRESET();
	cycles = cpu.step();

	cpu.S() = 0x300;
	cpu.pokeShort(0x300, 0x102C);	// Write return address
	board.poke(0xB00, 0x39);		// RTS
	cpu.PC() = 0xB00;
	cycles = cpu.step();

	BOOST_CHECK_EQUAL(cpu.PC(), 0x102C);
	BOOST_CHECK_EQUAL(cpu.S(), 0x302);
	BOOST_CHECK_EQUAL(cycles, 5);
}

BOOST_AUTO_TEST_SUITE_END()
